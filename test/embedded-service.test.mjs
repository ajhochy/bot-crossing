...
test('state.mark archives, restores and marks viewed without replacing unrelated state', async () => {
  const createEmbeddedService = requireService()
  await profiles(async ({ alpha }) => {
    const service = createEmbeddedService({ dataDir: alpha, scan: async () => [] })
    const initial = await readState(service)
    const seeded = await service.invoke('state.write', { state: { ...initial, settings: { theme: 'night' }, projectOverrides: { checkout: 'project' } }, baseUpdatedAt: initial.updatedAt })
    const archived = await service.invoke('state.mark', { threadId: 'codex:task-1', archived: true })
    assert.deepEqual(archived.archived, ['codex:task-1'])
    assert.ok(Number.isSafeInteger(archived.archivedAt['codex:task-1']))
    assert.deepEqual(archived.settings, seeded.settings)
    assert.deepEqual(archived.projectOverrides, seeded.projectOverrides)
    const viewed = await service.invoke('state.mark', { threadId: 'codex:task-1', viewedAt: 1234 })
    assert.equal(viewed.viewedAt['codex:task-1'], 1234)
    const restored = await service.invoke('state.mark', { threadId: 'codex:task-1', archived: false })
    assert.deepEqual(restored.archived, [])
    assert.equal(Object.hasOwn(restored.archivedAt, 'codex:task-1'), false)
    assert.equal(restored.viewedAt['codex:task-1'], 1234)
  })
})

test('a stale state.write conflicts after state.mark and three-way merge preserves the mark', async () => {
  const { mergeState } = await import('../src/game/merge-state.js')
  const createEmbeddedService = requireService()
  await profiles(async ({ alpha }) => {
    const service = createEmbeddedService({ dataDir: alpha, scan: async () => [] })
    const base = await readState(service)
    await service.invoke('state.mark', { threadId: 'codex:marked', archived: true })
    await assert.rejects(service.invoke('state.write', { state: { ...base, hiddenProjects: ['local-project'] }, baseUpdatedAt: base.updatedAt }), /conflict/i)
    const remote = await readState(service)
    const merged = mergeState(base, { ...base, hiddenProjects: ['local-project'] }, remote)
    assert.ok(merged.archived.includes('codex:marked'))
    const saved = await service.invoke('state.write', { state: merged, baseUpdatedAt: remote.updatedAt })
    assert.ok(saved.archived.includes('codex:marked'))
    assert.deepEqual(saved.hiddenProjects, ['local-project'])
  })
})
